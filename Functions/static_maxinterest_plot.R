static_maxinterest_plot_fun <- function(PlotData, Theme)
{
  ...
  static_maxinterest_plot <- (ggplot(data = PlotData  ,
                                      aes(x = ReserveSize,y = MaxInterestRate)) +
#                                  aes(x = ReserveSize,y = MaxInterestRate,fill = LoanType)) +
                                 geom_bar(fill = 'grey85',stat = 'identity', position = 'dodge', color = 'black', alpha = 0.85) +
...
#                                Theme )
return(static_maxinterest_plot)
}