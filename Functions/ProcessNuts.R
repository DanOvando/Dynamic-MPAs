ProcessNuts <- function(ReserveResults,
                        Fleet,
                        Scale_Yields,
                        npb_focus)
{
  if (Scale_Yields == T)
  {
    ...
    ReserveResults$YieldBalance <-
      ReserveResults$Yield - ReserveResults$SQYield
  ...
  ReserveResults$perc_yield_balance <-
    (ReserveResults$Yield - ReserveResults$SQYield) / ReserveResults$SQYield

  ReserveResults <- ReserveResults %>%
    group_by(Run) %>%
    arrange(Year) %>%
    mutate(
      PresentYield = Yield * (1 + Fleet$YieldDisc) ^ -(Year - 1),
      PresentBalance = (Yield - SQYield) * (1 + Fleet$YieldDisc) ^ -(Year - 1)
    ) %>%
    mutate(
      NPB = cumsum(PresentBalance),
      NB = cumsum(YieldBalance),
      perc_NB = cumsum(Yield) / cumsum(SQYield) - 1,
      perc_biomass =  (Biomass / SQBiomass) - 1,
      NPY = cumsum(PresentYield),
      RequestedLoan = sum(PresentBalance[YieldBalance < 0])
    ) %>%
    ungroup() %>%
    group_by(Species) %>%
    mutate(
      s_Yield = Yield / max(Yield, na.rm = T),
      s_PresentYield = s_Yield * (1 + Fleet$YieldDisc) ^ -(Year - 1)
    ) %>%
    ungroup() %>%
    group_by(Run) %>%
    arrange(Year) %>%
    mutate(
      net_yields = cumsum(Yield),
      net_biomass = cumsum(Biomass),
      s_NPY = cumsum(s_PresentYield),
      s_Balance = (Yield / SQYield),
    ...
    mutate(Utility = (1 - npb_focus) * (Biomass / max(Biomass[Year == max(Year)])) + (npb_focus) * (NPY /
                                                                                                      max(NPY[Year == max(Year)]))) %>%
  ...
  OptimalRun <- filter(ReserveResults, Year == max(Year)) %>%
    group_by(Species) %>%
    summarize(OptNPB = Run[NPB == max(NPB)][1],
              OptEQ = Run[net_yields == max(net_yields)][1],
              OptUtility = Run[Utility == max(Utility)][1])

  ReserveResults$Scenario <-
    with(ReserveResults,
         paste(FinalReserve, Intercept, Slope, sep = '-'))
  ...
  BestUnifiedScenario <-
    UnifiedRuns$Scenario[UnifiedRuns$TotalUtility == max(UnifiedRuns$TotalUtility)]

  ReserveResults$BestUnifiedRun <-
    ReserveResults$Scenario %in% BestUnifiedScenario

  ReserveResults$BestRun <-
    ReserveResults$Run %in% OptimalRun$OptUtility

  ResSummary <- ReserveResults %>%
    group_by(Run) %>%
    summarize(
      TimeToNPB = which(NPB >= 0)[1],
      Species = unique(Species),
      BestRun = unique(BestRun),
      FinalNPB = last(NPB),
      FinalNB = last(NB),
      Final_Perc_Biomass = last(perc_biomass),
  ...
  for (i in seq_len(length(RunNames)))
  {
    ...
    ResSummary$MaxInterestRate[i] <-
      100 * exp(
        optim(
          -4,
          FindMaxInterestRate,
          LoanTime = LoanTime,
          LoanAmount = -ResSummary$LoanNegativeYields[Where],
          Surplus = ResSummary$LoanAvailableSurplus[Where],
          lower = -10,
          upper = 10,
          method = 'Brent'
        )$par
  ...
  ResSummary$TimeToNPB[is.na(ResSummary$TimeToNPB)] <-
    max(ReserveResults$Year)
  ...
  species_comparison <-
    as.data.frame(matrix(
      NA,
      nrow = length(SpeciesList) * length(SpeciesList),
      ncol = 8
    ))

  colnames(species_comparison) <-
    c(
      'Species1',
      'Species2',
      'NPB',
  ...
  ResSummary$RunDescription <-
    paste(ResSummary$ReserveSize,
          ResSummary$Intercept,
          ResSummary$Slope,
          sep = '-')
  ...
  {
    Where <-
      ResSummary$Species == SpeciesList[i] & ResSummary$BestRun == T

    species_opt_res <- ResSummary$RunDescription[Where]

    FinalRes <-
      paste(100 * ResSummary$ReserveSize[Where], '%', sep = '')
    ...
    for (j in 1:length(SpeciesList))
    {
      ...
      Where2 <-
        ResSummary$Species == SpeciesList[j] &
        ResSummary$RunDescription == species_opt_res

      species_comparison[cc, ] <-
        data.frame(
          SpeciesList[i],
          SpeciesList[j],
  ...
  species_comparison <- species_comparison %>%
    ...
    group_by(Species2) %>%
    mutate(
      Percent_of_best_NPB = sign(max(NPB)) * (NPB / max(NPB) - 1),
